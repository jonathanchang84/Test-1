...
if not ml_df.empty:
    ...
    # 3. GENERATE FUTURE FORECAST HORIZON (30 Days Forward)
    min_time_unix = X_timestamps.min()
    max_time_unix = X_timestamps.max()
    thirty_days_seconds = 30 * 24 * 60 * 60
    future_time_unix = max_time_unix + thirty_days_seconds
    
    # Create a continuous timeline array spanning from the oldest historical point to 30 days in the future
    forecast_timeline_unix = np.linspace(min_time_unix, future_time_unix, 100).reshape(-1, 1)
    
    # Convert those unix numbers back to Datetime objects so Plotly can draw them cleanly
    forecast_timeline_dt = pd.to_datetime(forecast_timeline_unix.flatten(), unit='s')
    
    # Generate predictions across the entire expanded timeline (historical + future horizon)
    ...
    # Extended Red Predictive Trendline (reaches 30 days past the last data point)
    trend_fig.add_trace(go.Scatter(
        x=forecast_timeline_dt, y=extended_predictions.flatten(),
        mode='lines', name='30-Day Predictive Forecasting Trendline',
    ...
    trend_fig.update_layout(
        ...
        xaxis=dict(title="Timeline (Including 30-Day Forecast Window)"),
...
with edu_tabs[0]:
    ...
    The system asks a fundamental predictive question: *Based on the chronological timeline of recent seismic events, what is the mathematical trajectory of future events over the next 30 days?*
    To answer this, the system trains on your historical table data, maps the calculated slope, and then extends that slope line forward beyond your latest data parameters into empty future time coordinates.
    ...
    * **Statistical Forecasting Horizon (`.predict()`):** The trained model leverages its mathematical formula in memory ($Y = \\beta_0 + \\beta_1X$). The code generates a brand new array of timestamps stretching **30 days into the future**, calculates predictions for those unmapped dates, and plots them as the dashed **red Trendline** breaking out past your last data point.
    """)
...
with edu_tabs[1]:
    ...
    * **The Extrapolation:** Positioned inside the mathematical canvas, the dashed **red trajectory vector** leverages calculated historical slopes to compute future estimations, plotting data projections 30 days into the future relative to the newest row entry.
